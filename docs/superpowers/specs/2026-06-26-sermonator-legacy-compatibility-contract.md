...
| `per_page` (alias `posts_per_page`) | FAITHFUL **pending real pagination**; until then UNVALIDATABLE | `SermonQuery` count + paginated `Renderer` list variant + escaped pager on the **registered** `sermon_page` query var (added via the `query_vars` filter). The embedded current-page is read from `sermon_page` — **NOT** `GridArgs::currentPage()` (which reads the archive main query's `paged`/`page`, reserved for the archive) — and pager links use `add_query_arg('sermon_page', N)` on the current request URL, **NOT** pretty `/page/N/` permalinks (which collide with the main query and 404 on a static embedding page) | until the pager lands: **named in the notice AND fires `sermonator_list_truncated`** — a naive count map silently drops the long tail of a large archive (the native grid has no pager); certified faithful (notice dropped) only once pagination ships AND the integration test proves page 2 (via `sermon_page`) loses no content on a **NON-archive** embedding page |
