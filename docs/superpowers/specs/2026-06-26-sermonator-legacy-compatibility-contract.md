...
| 2026-06-27 | Bundle 2 T11 (prevalence counter) | `Migration\PrevalenceCounter` ships the §63 detect/verify prevalence rollup so the FIRST real migration sizes the deferred work. Records: podcasts-with-non-empty-scope, single-axis-scoped count, `>1`-published-podcast (bool), embedded-`[sermons]`/`[sermons_sm]` attribute density (posts, with-attributes, max, per-name histogram), and page-builder finding counts (reusing the `PageBuilderScanner` scan). Persisted to the new `Identifiers::OPTION_MIGRATION_PREVALENCE` ONLY on the write-gated `Orchestrator::detect()` + `Verifier::verify()` paths (podcast-scope counts populate at verify; source-content scans from detect) — never on a GET. The wizard report surface (`maybeRenderWizardNotice`) is a PURE READER of the persisted option, mirroring `OPTION_BIBLE_STATS`/`CoverageAudit`. |
